...
type ContentLoader struct {
	...
	cache map[gqlid.ContentID]*ent.Content
...
func (l *ContentLoader) Load(key gqlid.ContentID) (*ent.Content, error) {
...
func (l *ContentLoader) LoadThunk(key gqlid.ContentID) func() (*ent.Content, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*ent.Content, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ContentLoader) LoadAll(keys []gqlid.ContentID) ([]*ent.Content, []error) {
	results := make([]func() (*ent.Content, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	contents := make([]*ent.Content, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ContentLoader) LoadAllThunk(keys []gqlid.ContentID) func() ([]*ent.Content, []error) {
	results := make([]func() (*ent.Content, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ContentLoader) Prime(key gqlid.ContentID, value *ent.Content) bool {
	l.mu.Lock()
	var found bool
...
func (l *ContentLoader) Clear(key gqlid.ContentID) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *ContentLoader) unsafeSet(key gqlid.ContentID, value *ent.Content) {
	if l.cache == nil {
		l.cache = map[gqlid.ContentID]*ent.Content{}
	}
	l.cache[key] = value
}
...
func (b *contentLoaderBatch) keyIndex(l *ContentLoader, key gqlid.ContentID) int {
