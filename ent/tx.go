...
func (tx *Tx) init() {
	...
	tx.ContentEpisode = NewContentEpisodeClient(tx.config)
	tx.ContentProgress = NewContentProgressClient(tx.config)
	tx.Course = NewCourseClient(tx.config)
	tx.JournalRecord = NewJournalRecordClient(tx.config)
	tx.Lesson = NewLessonClient(tx.config)
	tx.LessonAudio = NewLessonAudioClient(tx.config)
	tx.LessonTechnique = NewLessonTechniqueClient(tx.config)
	tx.Technique = NewTechniqueClient(tx.config)
	tx.Unit = NewUnitClient(tx.config)
	tx.UnitTechnique = NewUnitTechniqueClient(tx.config)
