...
type ContentQuery struct {
	...
	withEpisodes      *ContentEpisodeQuery
	loadTotal         []func(context.Context, []*Content) error
	modifiers         []func(*sql.Selector)
	withNamedEpisodes map[string]*ContentEpisodeQuery
...
func (cq *ContentQuery) QueryEpisodes() *ContentEpisodeQuery {
	query := (&ContentEpisodeClient{config: cq.config}).Query()
...
func (cq *ContentQuery) FirstID(ctx context.Context) (id gqlid.ContentID, err error) {
	var ids []gqlid.ContentID
...
func (cq *ContentQuery) FirstIDX(ctx context.Context) gqlid.ContentID {
...
func (cq *ContentQuery) OnlyID(ctx context.Context) (id gqlid.ContentID, err error) {
	var ids []gqlid.ContentID
...
func (cq *ContentQuery) OnlyIDX(ctx context.Context) gqlid.ContentID {
...
func (cq *ContentQuery) IDs(ctx context.Context) (ids []gqlid.ContentID, err error) {
...
func (cq *ContentQuery) IDsX(ctx context.Context) []gqlid.ContentID {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *ContentQuery) WithEpisodes(opts ...func(*ContentEpisodeQuery)) *ContentQuery {
	query := (&ContentEpisodeClient{config: cq.config}).Query()
...
func (cq *ContentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Content, error) {
	...
	if query := cq.withEpisodes; query != nil {
		if err := cq.loadEpisodes(ctx, query, nodes,
			func(n *Content) { n.Edges.Episodes = []*ContentEpisode{} },
			func(n *Content, e *ContentEpisode) { n.Edges.Episodes = append(n.Edges.Episodes, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedEpisodes {
		if err := cq.loadEpisodes(ctx, query, nodes,
			func(n *Content) { n.appendNamedEpisodes(name) },
			func(n *Content, e *ContentEpisode) { n.appendNamedEpisodes(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *ContentQuery) loadEpisodes(ctx context.Context, query *ContentEpisodeQuery, nodes []*Content, init func(*Content), assign func(*Content, *ContentEpisode)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[gqlid.ContentID]*Content)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(contentepisode.FieldContentID)
	}
	query.Where(predicate.ContentEpisode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(content.EpisodesColumn), fks...))
	}))
...
func (cq *ContentQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(content.Table, content.Columns, sqlgraph.NewFieldSpec(content.FieldID, field.TypeInt64))
	_spec.From = cq.sql
...
func (cq *ContentQuery) WithNamedEpisodes(name string, opts ...func(*ContentEpisodeQuery)) *ContentQuery {
	query := (&ContentEpisodeClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedEpisodes == nil {
		cq.withNamedEpisodes = make(map[string]*ContentEpisodeQuery)
	}
	cq.withNamedEpisodes[name] = query
	return cq
}
